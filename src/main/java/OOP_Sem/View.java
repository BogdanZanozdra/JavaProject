...
public class View {
    ...
    private static String getChar(int x, int y){
        String out = "| ";
        for (Unit unit:  main.all) {
            if (unit.getCoords()[0] == x && unit.getCoords()[1] == y){
                if (unit.getHp() <= 0) {
                    out = "|" + (AnsiColors.ANSI_RED + unit.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                    break;
                }
                if (main.team1.contains(unit)) out = "|" + (AnsiColors.ANSI_GREEN + unit.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                if (main.team2.contains(unit)) out = "|" + (AnsiColors.ANSI_BLUE + unit.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                break;
            }
        }
        return out;
